...
import { fetchVendorRequirements } from '@/services/properties';
...
export default function VendorDetail() {
  ...
  const { data: certificates } = useQuery({
    queryKey: ['certificates', 'vendor', id],
    queryFn: () => fetchCertificates('vendor', id!),
    enabled: !!id,
  });

  const { data: emailLog } = useQuery({
    queryKey: ['emailLog', 'vendor', id],
    queryFn: () => fetchEmailLog('vendor', id!),
    enabled: !!id,
  });

  const { data: requirements } = useQuery({
    queryKey: ['vendorRequirements', vendor?.property_id],
    queryFn: () => fetchVendorRequirements(vendor!.property_id!),
    enabled: !!vendor?.property_id,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (updates: { name?: string; email?: string }) => updateVendor(id!, updates),
    onSuccess: () => {
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const filePath = await uploadCOIFile('vendor', id, file);

        // Run compliance check
        let result: ComplianceResult | null = null;
        if (requirements && vendor.property) {
          ...
          result = checkCompliance({
            ...
            property: {
              additional_insured_entities: vendor.property.additional_insured_entities ?? [],
              certificate_holder_name: vendor.property.certificate_holder_name ?? null,
              loss_payee_entities: vendor.property.loss_payee_entities ?? [],
            },
          });
        }
        ...
        await updateVendor(id, {
          status: (result?.overall_status as 'compliant' | 'non_compliant' | 'expired') ?? 'pending',
          expiration_date: earliest ?? undefined,
        } as Record<string, unknown>);

        queryClient.invalidateQueries({ queryKey: ['vendor', id] });
        queryClient.invalidateQueries({ queryKey: ['certificates', 'vendor', id] });
    ...
    [vendor, id, requirements, queryClient]
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {vendor.email || vendor.contact_email || 'No email'} · {vendor.property?.name || 'No property'}
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          {vendor.upload_token && (
            <Button variant="outline" size="sm" asChild>
              <Link to={`/upload/${vendor.upload_token}`} target="_blank">
          ...
          <Button
            ...
            onClick={() => {
              ...
              setEditEmail(vendor.email || vendor.contact_email || '');
            }}
          >
